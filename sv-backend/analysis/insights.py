...
def generate_insights(
    statistics: dict, attendance_data: dict | None, score_cols: list
) -> list[str]:
    insights: list[str] = []
    ...
    if failing:
        ...
        insights.append(f'Subjects with failing averages: {subject_list}.')
        insights.append(
            'More than 40% of students are below the passing threshold — targeted intervention is urgent.'
        )
    ...
    if red_count > 0:
        ...
        insights.append(
            f'{red_count} student{plural} ({red_percentage}% of the class) fall in the at-risk category '
            f'with an average below 50%.'
        )
        insights.append('These students require immediate personalised support.')

    if attendance_data and attendance_data.get('difference') is not None:
        difference = attendance_data['difference']
        high_avg = attendance_data['high_attendance_avg']
        low_avg = attendance_data['low_attendance_avg']
        insights.append(
            f'Students with high attendance (80%+) average {high_avg:.1f}%, '
            f'while those with low attendance (below 60%) average {low_avg:.1f}%. '
            f'The {difference:.1f} point gap suggests a strong correlation between attendance and performance.'
        )
...
def generate_recommendations(
    ...
    if attendance_data and attendance_data.get('difference') is not None:
        if attendance_data['difference'] > 10:
            recs.append(
                f"Launch an attendance awareness campaign. The data shows a "
                f"{attendance_data['difference']:.1f}-point score advantage "
